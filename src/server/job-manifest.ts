...
function buildEnvVars(
  ...
  // Hermes-specific: prevent project config pollution, always set after user overrides
...
/**
 * Build the Hermes runtime config JSON for permission.external_directory=allow.
...
export function buildJobManifest(input: JobBuildInput): JobBuildResult {
  ...
  // Job naming - using agent-hermes- prefix as requested
  const agentSlug = sanitizeForK8sName(agent.id);
  const runSlug = sanitizeForK8sName(runId);
  const jobName = `agent-hermes-${agentSlug}-${runSlug}`;
  ...
  if (variant) hermesArgs.push("--variant", variant);
  if (extraArgs.length > 0) hermesArgs.push(...extraArgs);
  ...
  // 1. Optionally write hermes runtime config for permission bypass
  // 2. Pipe prompt into hermes
  const hermesArgsEscaped = hermesArgs.map((a) => `'${a.replace(/'/g, "'\\''")}'`).join(" ");
  const configSetup = runtimeConfigJson
    ? `mkdir -p ~/.config/hermes && echo '${runtimeConfigJson.replace(/'/g, "'\\''")}' > ~/.config/hermes/hermes.json && `
    : "";
  const mainCommand = `${configSetup}cat /tmp/prompt/prompt.txt | hermes ${hermesArgsEscaped}`;
  ...
  return { job, jobName, namespace, prompt, hermesArgs, promptMetrics };
}
